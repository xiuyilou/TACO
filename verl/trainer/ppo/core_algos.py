...
def compute_taco_token_advantages(
    ...
    """Apply Tail-Aware Credit Calibration (TACO) to positive token advantages."""
    if alpha <= 0.0:
        raise ValueError(f"taco_alpha must be > 0, got {alpha}")
    if not (0.0 <= lambda_ <= 1.0):
        raise ValueError(f"taco_lambda must be in [0, 1], got {lambda_}")

    with torch.no_grad():
        mask = response_mask.bool()
        tail_risk = -log_prob - entropy + math.log(alpha)
        tail_risk = torch.where(mask, tail_risk, torch.zeros_like(tail_risk))

        tail_positive_mask = (tail_risk > 0) & mask
        weights = torch.ones_like(advantages)
        weights = torch.where(
            tail_positive_mask,
            1.0 - lambda_ * (1.0 - torch.exp(-tail_risk)),
            weights,
        )

        positive_adv_mask = (advantages > 0) & mask
        taco_advantages = torch.where(positive_adv_mask, weights * advantages, advantages)

        masked_weights = torch.where(mask, weights, torch.zeros_like(weights))
        masked_tail_risk = torch.where(mask, tail_risk, torch.zeros_like(tail_risk))
        metrics = {
            "weight_mean": verl_F.masked_mean(masked_weights, response_mask),
            "tail_risk_mean": verl_F.masked_mean(masked_tail_risk, response_mask),
            "tail_token_frac": verl_F.masked_mean(tail_positive_mask.float(), response_mask),
            "positive_adv_token_frac": verl_F.masked_mean(positive_adv_mask.float(), response_mask),
        }

    return taco_advantages, metrics
