...
def get_colocate_zmq_handle(device_uuid: str) -> str:
    """Build a filesystem-backed ZMQ IPC handle for rollout weight sync.

    The old implementation hard-coded a user-specific path such as
    `/path/to/runtime_tmp`, which breaks as soon as the runtime uses a
    different home directory. Default to the system temp dir instead and
    allow an override via `VERL_ZMQ_IPC_DIR`.
    """
    socket_dir = os.environ.get("VERL_ZMQ_IPC_DIR")
    if not socket_dir:
        socket_dir = os.path.join(tempfile.gettempdir(), "verl-zmq")
    socket_dir = os.path.abspath(socket_dir)
    os.makedirs(socket_dir, exist_ok=True)
    socket_path = os.path.join(socket_dir, f"rl-colocate-zmq-{device_uuid}.sock")
...
class vLLMColocateWorkerExtension:
    ...
    def _get_zmq_handle(self) -> str:
        ...
        return get_colocate_zmq_handle(self.device_uuid)
