...
class vLLMHttpServer:
    ...
    async def generate(
        ...
        _logprobs_val = sampling_params.pop("logprobs", False)
        if isinstance(_logprobs_val, int) and _logprobs_val > 0:
            sampling_params["logprobs"] = _logprobs_val
        elif _logprobs_val:
            sampling_params["logprobs"] = 0
        else:
            sampling_params["logprobs"] = None
        # Drop VERL-side rollout controls that are not native vLLM SamplingParams fields.
        sampling_params.pop("adaptive_temperature", None)
        sampling_params.pop("temperature_tau", None)
        sampling_params.pop("norm_entropys", None)
        ...
        log_probs = None
        log_probs = None
        topk_indices = getattr(final_res.outputs[0], "topk_indices", None)
        flat_topk_indices = getattr(final_res.outputs[0], "flat_topk_indices", None)
        support_counts = getattr(final_res.outputs[0], "support_counts", None)
        support_offsets = getattr(final_res.outputs[0], "support_offsets", None)
        if flat_topk_indices is None and topk_indices is not None and support_counts is not None:
            topk_indices = topk_indices.to(device="cpu", dtype=torch.long)
            support_counts = support_counts.to(device="cpu", dtype=torch.int32).reshape(-1)
            support_offsets = torch.zeros(support_counts.numel() + 1, dtype=torch.int32)
            support_offsets[1:] = torch.cumsum(support_counts, dim=0)
            if topk_indices.numel() > 0:
                valid_mask = (
                    torch.arange(topk_indices.shape[-1], device=topk_indices.device).unsqueeze(0)
                    < support_counts.unsqueeze(1)
                )
                flat_topk_indices = topk_indices[valid_mask]
            else:
                flat_topk_indices = torch.empty(0, dtype=torch.long)
            topk_indices = None
            support_counts = None
        elif flat_topk_indices is not None:
            flat_topk_indices = flat_topk_indices.to(device="cpu", dtype=torch.long).reshape(-1)
            if support_offsets is not None:
                support_offsets = support_offsets.to(device="cpu", dtype=torch.int32).reshape(-1)
            topk_indices = None
            support_counts = None
        feasible_set_entropy_coeff = getattr(final_res.outputs[0], "feasible_set_entropy_coeff", None)
        feasible_set_prob_cap = getattr(final_res.outputs[0], "feasible_set_prob_cap", None)
        top_k_logprobs = None
        if sampling_params.logprobs is not None:
            log_probs = [logprobs[token_ids[i]].logprob for i, logprobs in enumerate(final_res.outputs[0].logprobs)]
            if sampling_params.logprobs > 0:
                top_k_logprobs = [
                    {tid: lp.logprob for tid, lp in logprobs.items()}
        ...
        return TokenOutput(
            ...
            flat_topk_indices=flat_topk_indices,
            support_counts=support_counts,
            support_offsets=support_offsets,
            feasible_set_entropy_coeff=feasible_set_entropy_coeff,
            feasible_set_prob_cap=feasible_set_prob_cap,
