...
class TokenOutput(BaseModel):
    ...
    """kept token ids per position for keep-sampling-mask style replay"""
    flat_topk_indices: Optional[torch.Tensor] = None
    """flattened kept token ids across positions for ragged replay"""
    support_counts: Optional[torch.Tensor] = None
    """number of valid kept token ids per position"""
    support_offsets: Optional[torch.Tensor] = None
    """exclusive prefix offsets aligned with flattened kept token ids"""
    keep_flags: Optional[torch.Tensor] = None
    """legacy valid flags aligned with topk_indices"""
    feasible_set_entropy_coeff: Optional[float] = None
    """feasible-set entropy coefficient used during rollout"""
    feasible_set_prob_cap: Optional[float] = None
    """feasible-set probability cap used during rollout"""
    top_k_logprobs: Optional[list[dict[int, float]]] = None
    """top-k logprobs at each token position for varentropy"""
