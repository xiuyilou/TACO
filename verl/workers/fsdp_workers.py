...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        calculate_varentropy = data.meta_info.pop("calculate_varentropy", calculate_entropy)
        calculate_shape_stats = data.meta_info.pop("calculate_shape_stats", calculate_varentropy)
        with self.ulysses_sharding_manager:
            with adapter_ctx:
            ...
            if calculate_varentropy and "varentropys" in outputs:
                tensors["varentropys"] = outputs["varentropys"]
            if calculate_shape_stats and "h1" in outputs:
                tensors["h1"] = outputs["h1"]
                tensors["h2"] = outputs["h2"]
            if "sum_pi_squared" in outputs:
                tensors["sum_pi_squared"] = outputs["sum_pi_squared"]
            if "topk_indices" in outputs:
                tensors["topk_indices"] = outputs["topk_indices"]
            if "keep_flags" in outputs:
                tensors["keep_flags"] = outputs["keep_flags"]
